...
class absenteeism_model():
        ...
        def load_and_clean_data(self, data_file):
            ...
            reason_type_1 = reason_columns.loc[:, 1:14].max(axis=1).fillna(0).astype(int).rename('Reason_1')
            reason_type_2 = reason_columns.loc[:, 15:17].max(axis=1).fillna(0).astype(int).rename('Reason_2')
            reason_type_3 = reason_columns.loc[:, 18:21].max(axis=1).fillna(0).astype(int).rename('Reason_3')
            reason_type_4 = reason_columns.loc[:, 22:].max(axis=1).fillna(0).astype(int).rename('Reason_4')
